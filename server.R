...
shinyServer(function(input, output) {
  output$user_data <- renderText({
    ...
    paste0("<strong>","Your weight is ", input$weight, " kg and your height is ", input$height, " cm", "</strong>")
  })
  
  output$results <- renderText({
    bmi = round(input$weight/(input$height/100)^2, 1)
    ...
    else if (bmi <= 18.5) cond = "<span style='color: blue'>Underweight</span>"
    else if (bmi <= 25.0) cond = "<span style='color: green'>Normal</span>"
    else if (bmi <= 30.0) cond = "<span style='color: blue'>Overweight</span>"
